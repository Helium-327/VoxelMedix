...
*      DESCRIPTION: 
*      VERSION: v1.0
*      FEATURES: Attention Unet 3D + ResConv + DenseASPP(BottleNeck) + AnisotropicConv3D
=================================================
'''
...
class SKConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, 
                 ...
                 dilation=1, groups=1, reduction=16):
        """
        参数说明：
        - kernel_size: 多分支的卷积核尺寸列表（默认包含3x3x3和5x5x5）
        - reduction: 通道压缩比例（用于注意力计算）
        """
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Linear(out_channels // reduction, len(kernel_size) * out_channels),
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, x):
        # 多分支卷积
        feats = [conv(x) for conv in self.convs]  # 各分支输出列表
        
        # 特征融合
        batch_size = x.size(0)
        feats = torch.stack(feats, dim=0)  # [num_branches, B, C, D, H, W]
        
        # 计算通道注意力
        channel_attn = self.avg_pool(sum(feats)).view(batch_size, self.out_channels)
        channel_attn = self.fc(channel_attn).view(batch_size, len(self.kernel_size), 
                                                self.out_channels, 1, 1, 1)
        
        # 特征加权融合
        feats = (feats * channel_attn).sum(dim=0)  # [B, C, D, H, W]
        return feats
...
class ResConv3D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = nn.Sequential(
            SKConv3d(in_channels, out_channels),
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True),
            SKConv3d(out_channels, out_channels),
            nn.BatchNorm3d(out_channels)
        )
