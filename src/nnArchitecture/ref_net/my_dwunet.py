...
=================================================
'''
# v2:计划基于Attention UNet3D改进，使用dw卷积减少参数量

from turtle import forward
from matplotlib.pylab import vonmises
import torch
...
class ResBlock3D(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, 
        ...
        self.conv2 = nn.Sequential(
            conv_op(out_channels, out_channels, kernel_size=3,padding=1, bias=False),
            nn.BatchNorm3d(out_channels))
...
class DenseASPP3D(nn.Module):
    def __init__(self, in_channels, out_channels, rates=[1, 2, 4, 8], conv_op=nn.Conv3d):
        super(DenseASPP3D, self).__init__()
        self.aspp1 = nn.Sequential(
            conv_op(in_channels, out_channels//4, kernel_size=3, padding=rates[0], dilation=rates[0]),
            nn.BatchNorm3d(out_channels//4),
            nn.ReLU(inplace=True)
        )
        self.aspp2 = nn.Sequential(
            conv_op(in_channels+out_channels//4, out_channels//4, kernel_size=3, padding=rates[1], dilation=rates[1]),
            nn.BatchNorm3d(out_channels//4),
            nn.ReLU(inplace=True)
        )
        self.aspp3 = nn.Sequential(
            conv_op(in_channels + 2*(out_channels//4), out_channels//4, kernel_size=3, padding=rates[2], dilation=rates[2]),
            nn.BatchNorm3d(out_channels//4),
            nn.ReLU(inplace=True)
        )
        self.aspp4 = nn.Sequential(
            conv_op(in_channels + 3*(out_channels//4), out_channels//4, kernel_size=3, padding=rates[3], dilation=rates[3]),
            nn.BatchNorm3d(out_channels//4),
            nn.ReLU(inplace=True)
        )
        self.global_avg = nn.Sequential(
            nn.AdaptiveAvgPool3d(1),
            conv_op(in_channels, out_channels//4, kernel_size=1),
            nn.ReLU(inplace=True)
        )
        self.fusion = conv_op(5*(out_channels//4), out_channels, 1)
...
class DecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, convTranspose=True, conv_op=nn.Conv3d):
        super(DecoderBlock, self).__init__()
        if convTranspose:
            self.up = nn.ConvTranspose3d(in_channels=in_channels, out_channels=in_channels,kernel_size=4,stride=2, padding=1)
        else:
            self.up = nn.Upsample(scale_factor=2)
 
        self.Conv = ResBlock3D(in_channels, out_channels, conv_op=conv_op)
...
class Proposed_DWUNet(nn.Module):
    """
    ...
    def __init__(self,
                 ...
                 convTranspose=True,
                 conv_op=DWConv):
        super(Proposed_DWUNet, self).__init__()
 
        self.Maxpool = nn.MaxPool3d(kernel_size=2, stride=2)
 
        self.decoder1 = ResBlock3D(in_channels=in_channel, out_channels=channel_list[0], conv_op=conv_op)
        self.decoder2 = ResBlock3D(in_channels=channel_list[0], out_channels=channel_list[1], conv_op=conv_op)
        self.decoder3 = ResBlock3D(in_channels=channel_list[1], out_channels=channel_list[2], conv_op=conv_op)
        self.decoder4 = ResBlock3D(in_channels=channel_list[2], out_channels=channel_list[3], conv_op=conv_op)
        
        self.BottleNeck = DenseASPP3D(in_channels=channel_list[3], out_channels=channel_list[4], conv_op=conv_op)
        ...
        self.Up4 = DecoderBlock(in_channels=channel_list[3], 
        ...
        self.Up3 = DecoderBlock(in_channels=channel_list[2], 
        ...
        self.Up2 = DecoderBlock(in_channels=channel_list[1], 
