...
class BaseDWUNet3D(nn.Module):
    def __init__(self, in_channels, out_channels, channels_list, soft=False):
        ...
        # Placeholder for specific block types
        self.encoder = BaseDWUNet3D._Block # 加载定义的静态类方法
        # self.bottleneck_block = None
        # self.decoder = None
        ch1, ch2, ch3, ch4 = channels_list
        if soft:
    ...
    def _Block(in_channels, features, kernel_size, padding, name):
        return nn.Sequential(
            ...
            nn.Conv3d(features, features, (kernel_size, 1, 1), (padding, 0, 0), bias=False),
            nn.Conv3d(features, features, (1, kernel_size, 1), (0, padding, 0), bias=False),
            nn.Conv3d(features, features, (1, 1, kernel_size), (0, 0, padding), bias=False),
            nn.BatchNorm3d(num_features=features),
            nn.GELU(inplace=True),
            SE_Block(features),
            # TODO: 添加DW卷积块
...
if __name__ == '__main__':
    ...
    # model = soft_UNet3D(in_channels=4, out_channels=4).to(device)3333
