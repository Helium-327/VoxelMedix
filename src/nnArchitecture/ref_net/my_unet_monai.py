...
class ConvBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        conv_output: torch.Tensor = self.conv(x)
        return conv_output
...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        resConv_output = self.res_block(x)
        return resConv_output
...
class Proposed_UNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoder1 = EncoderBlock(ch_in=in_channel, ch_out=channel_list[0])
        self.decoder2 = EncoderBlock(ch_in=channel_list[0], ch_out=channel_list[1])
        self.decoder3 = EncoderBlock(ch_in=channel_list[1], ch_out=channel_list[2])
        self.decoder4 = EncoderBlock(ch_in=channel_list[2], ch_out=channel_list[3])
        self.decoder5 = EncoderBlock(ch_in=channel_list[3], ch_out=channel_list[4])

        # self.bottle_neck = Inception_Block(channel_list[4], channel_list[4])  # TODO: 改进2：BottleNeck使用Inception Block
        self.Up5 = DecoderBlock(ch_in=channel_list[4], ch_out=channel_list[3], convTranspose=convTranspose)
        ...
        self.DecoderBlock5 = EncoderBlock(ch_in=channel_list[4],
                                   ch_out=channel_list[3])
 
        self.Up4 = DecoderBlock(ch_in=channel_list[3], ch_out=channel_list[2], convTranspose=convTranspose)
        self.skip3 = CGGA(channel_list[2])
        self.Att4 = Attention_block(F_g=channel_list[2],
                                    F_l=channel_list[2],
                                    F_int=channel_list[1])
        self.DecoderBlock4 = EncoderBlock(ch_in=channel_list[3],
                                   ch_out=channel_list[2])
 
        self.Up3 = DecoderBlock(ch_in=channel_list[2], ch_out=channel_list[1], convTranspose=convTranspose)
        self.skip2 = CGGA(channel_list[1])
        self.Att3 = Attention_block(F_g=channel_list[1],
                                    F_l=channel_list[1],
                                    F_int=64)
        self.DecoderBlock3 = EncoderBlock(ch_in=channel_list[2],
                                   ch_out=channel_list[1])
 
        self.Up2 = DecoderBlock(ch_in=channel_list[1], ch_out=channel_list[0], convTranspose=convTranspose)
        self.skip1 = CGGA(channel_list[0])
        self.Att2 = Attention_block(F_g=channel_list[0],
                                    F_l=channel_list[0],
                                    F_int=channel_list[0] // 2)
        self.DecoderBlock2 = EncoderBlock(ch_in=channel_list[1],
